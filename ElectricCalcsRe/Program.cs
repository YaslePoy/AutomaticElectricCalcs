...
namespace ElectricCalcs
{
    ...
    public class Program
    {
        ...
        public static void WriteDecision(double n, double m)
        {
            Writer.Reset();
            Console.WriteLine($"-- Calulating of capitor model for N {n} M {m} --");
            Dictionary<string, double> repls = new Dictionary<string, double>();
            repls.Add("C1", m + n);
            ...
            ModelParcer parcer = new ModelParcer("shemeCapLabElvin.txt", repls);
            var model = parcer.ParceUniversal();
            model.CalculateValues();
            Writer.Reset();
            repls = new Dictionary<string, double>();
            repls.Add("R1", m);
            repls.Add("R2", n);
            repls.Add("R3", m / 2);
            repls.Add("R4", n / 3);
            ...
            if (n % 2 == 0)
                ...
                repls.Add("Ia", Math.Abs(m - n) + 1);
            repls.ToList().ForEach(x => Console.WriteLine($"{x.Key} = {x.Value}"));
            parcer = new ModelParcer("sheme2.txt", repls);
            model = parcer.ParceUniversal();
            if (repls.Last().Key != "U7")
            {
                if (repls.Last().Key == "Ua")
                    model.U = repls.Last().Value;
